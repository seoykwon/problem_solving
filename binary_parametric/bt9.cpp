...
/**
 * 소형 패키지가 대형 패키지보다 단가가 낮다면, 소형 패키지로만 부족분을 채웁니다.
(missing + Sk[i] - 1) / Sk[i]는 소형 패키지가 몇 개 필요한지 계산하는 수식입니다.
예를 들어, Sk[i] = 4이고, missing = 9라면 소형 패키지 3개가 필요합니다.
(9 + 4 - 1) / 4 = 3 (올림 효과).
missing / Sk[i]로 계산할 경우, 나머지가 있는 상황에서 패키지가 부족하게 계산될 수 있기 때문에, +Sk[i]-1을 추가해 올림 처리를 합니다.
결과적으로 필요한 소형 패키지 개수에 가격 Sp[i]를 곱해 비용을 추가합니다.
 */
